...
export const ApplicationDetail: React.FC<ApplicationDetailProps> = ({ organizationName, applications, onBack }) => {
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      <div className="p-6 border-b border-gray-100 flex justify-between items-center">
        <div>
          <h2 className="text-xl font-bold text-[#1e293b]">Applicants for {organizationName}</h2>
          <p className="text-sm text-gray-400 mt-1">Detailed list of individual candidates</p>
        </div>
        <button 
          onClick={onBack}
          ...
          <tbody className="divide-y divide-gray-100">
            {applications.map((app) => (
              <tr key={app.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 font-semibold text-sm text-gray-900">{app.name}</td>
                ...
                <td className="px-6 py-4">
                  {app.hasCV ? (
                    <button className="text-indigo-600 hover:underline text-sm font-medium">View CV</button>
                  ) : (
